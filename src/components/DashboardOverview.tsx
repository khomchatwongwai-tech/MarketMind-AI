  ...
  const keyMarketIndices = [
    {
      name: 'S&P 500',
      symbol: 'SPX',
      price: isFiniteMarketNumber(rtMacroQuotes['SPX']?.price) ? `$${rtMacroQuotes['SPX']!.price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : quote.ticker === 'SPY' && isFiniteMarketNumber(quote.price) ? `$${quote.price.toFixed(2)}` : 'Unavailable',
      change: isFiniteMarketNumber(rtMacroQuotes['SPX']?.changePercent) ? `${rtMacroQuotes['SPX']!.changePercent >= 0 ? '+' : ''}${rtMacroQuotes['SPX']!.changePercent.toFixed(2)}%` : isFiniteMarketNumber(quote.changePercent) ? `${isPositive ? '+' : ''}${quote.changePercent.toFixed(2)}%` : 'Unavailable',
      isUp: isFiniteMarketNumber(rtMacroQuotes['SPX']?.change) ? rtMacroQuotes['SPX']!.change >= 0 : isPositive,
    },
    {
      name: 'NASDAQ 100',
      symbol: 'NDX',
      price: isFiniteMarketNumber(rtMacroQuotes['NDX']?.price) ? `$${rtMacroQuotes['NDX']!.price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : 'Unavailable',
      change: isFiniteMarketNumber(rtMacroQuotes['NDX']?.changePercent) ? `${rtMacroQuotes['NDX']!.changePercent >= 0 ? '+' : ''}${rtMacroQuotes['NDX']!.changePercent.toFixed(2)}%` : 'Unavailable',
      isUp: isFiniteMarketNumber(rtMacroQuotes['NDX']?.change) ? rtMacroQuotes['NDX']!.change >= 0 : true,
    },
    {
      name: 'DOW JONES',
      symbol: 'DJI',
      price: isFiniteMarketNumber(rtMacroQuotes['DJI']?.price) ? `$${rtMacroQuotes['DJI']!.price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : 'Unavailable',
      change: isFiniteMarketNumber(rtMacroQuotes['DJI']?.changePercent) ? `${rtMacroQuotes['DJI']!.changePercent >= 0 ? '+' : ''}${rtMacroQuotes['DJI']!.changePercent.toFixed(2)}%` : 'Unavailable',
      isUp: isFiniteMarketNumber(rtMacroQuotes['DJI']?.change) ? rtMacroQuotes['DJI']!.change >= 0 : true,
    },
    {
      name: 'SPY ETF',
      symbol: 'SPY',
      price: quote.ticker === 'SPY' && isFiniteMarketNumber(quote.price) ? `$${quote.price.toFixed(2)}` : isFiniteMarketNumber(rtMacroQuotes['SPY']?.price) ? `$${rtMacroQuotes['SPY']!.price.toFixed(2)}` : 'Unavailable',
      change: quote.ticker === 'SPY' && isFiniteMarketNumber(quote.changePercent) ? `${isPositive ? '+' : ''}${quote.changePercent.toFixed(2)}%` : isFiniteMarketNumber(rtMacroQuotes['SPY']?.changePercent) ? `${rtMacroQuotes['SPY']!.changePercent >= 0 ? '+' : ''}${rtMacroQuotes['SPY']!.changePercent.toFixed(2)}%` : 'Unavailable',
      isUp: quote.ticker === 'SPY' ? isPositive : isFiniteMarketNumber(rtMacroQuotes['SPY']?.change) ? rtMacroQuotes['SPY']!.change >= 0 : true,
    },
    {
      name: 'QQQ ETF',
      symbol: 'QQQ',
      price: quote.ticker === 'QQQ' && isFiniteMarketNumber(quote.price) ? `$${quote.price.toFixed(2)}` : isFiniteMarketNumber(rtMacroQuotes['QQQ']?.price) ? `$${rtMacroQuotes['QQQ']!.price.toFixed(2)}` : 'Unavailable',
      change: quote.ticker === 'QQQ' && isFiniteMarketNumber(quote.changePercent) ? `${isPositive ? '+' : ''}${quote.changePercent.toFixed(2)}%` : isFiniteMarketNumber(rtMacroQuotes['QQQ']?.changePercent) ? `${rtMacroQuotes['QQQ']!.changePercent >= 0 ? '+' : ''}${rtMacroQuotes['QQQ']!.changePercent.toFixed(2)}%` : 'Unavailable',
      isUp: quote.ticker === 'QQQ' ? isPositive : isFiniteMarketNumber(rtMacroQuotes['QQQ']?.change) ? rtMacroQuotes['QQQ']!.change >= 0 : true,
    },
    {
      name: 'VIX VOLATILITY',
      symbol: 'VIX',
      price: isFiniteMarketNumber(rtMacroQuotes['VIX']?.price) ? `${rtMacroQuotes['VIX']!.price.toFixed(2)}` : 'Unavailable',
      change: isFiniteMarketNumber(rtMacroQuotes['VIX']?.changePercent) ? `${rtMacroQuotes['VIX']!.changePercent >= 0 ? '+' : ''}${rtMacroQuotes['VIX']!.changePercent.toFixed(2)}%` : 'Unavailable',
      isUp: isFiniteMarketNumber(rtMacroQuotes['VIX']?.change) ? rtMacroQuotes['VIX']!.change >= 0 : false,
    },
    {
      name: '10Y TREASURY',
      symbol: 'US10Y',
      price: isFiniteMarketNumber(rtMacroQuotes['US10Y']?.price) ? `${rtMacroQuotes['US10Y']!.price.toFixed(2)}%` : 'Unavailable',
      change: isFiniteMarketNumber(rtMacroQuotes['US10Y']?.change) ? `${rtMacroQuotes['US10Y']!.change >= 0 ? '+' : ''}${rtMacroQuotes['US10Y']!.change.toFixed(2)}` : 'Unavailable',
      isUp: isFiniteMarketNumber(rtMacroQuotes['US10Y']?.change) ? rtMacroQuotes['US10Y']!.change >= 0 : false,
    },
    {
      name: 'WTI CRUDE OIL',
      symbol: 'CL',
      price: isFiniteMarketNumber(rtMacroQuotes['CL']?.price) ? `$${rtMacroQuotes['CL']!.price.toFixed(2)}` : 'Unavailable',
      change: isFiniteMarketNumber(rtMacroQuotes['CL']?.changePercent) ? `${rtMacroQuotes['CL']!.changePercent >= 0 ? '+' : ''}${rtMacroQuotes['CL']!.changePercent.toFixed(2)}%` : 'Unavailable',
      isUp: isFiniteMarketNumber(rtMacroQuotes['CL']?.change) ? rtMacroQuotes['CL']!.change >= 0 : false,
    },
    {
      name: 'GOLD SPOT',
      symbol: 'XAU',
      price: isFiniteMarketNumber(rtMacroQuotes['XAU']?.price) ? `$${rtMacroQuotes['XAU']!.price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : 'Unavailable',
      change: isFiniteMarketNumber(rtMacroQuotes['XAU']?.changePercent) ? `${rtMacroQuotes['XAU']!.changePercent >= 0 ? '+' : ''}${rtMacroQuotes['XAU']!.changePercent.toFixed(2)}%` : 'Unavailable',
      isUp: isFiniteMarketNumber(rtMacroQuotes['XAU']?.change) ? rtMacroQuotes['XAU']!.change >= 0 : true,
    },
    {
      name: 'BITCOIN',
      symbol: 'BTC',
      price: isFiniteMarketNumber(rtMacroQuotes['BTC']?.price) ? `$${rtMacroQuotes['BTC']!.price.toLocaleString('en-US', { maximumFractionDigits: 0 })}` : 'Unavailable',
      change: isFiniteMarketNumber(rtMacroQuotes['BTC']?.changePercent) ? `${rtMacroQuotes['BTC']!.changePercent >= 0 ? '+' : ''}${rtMacroQuotes['BTC']!.changePercent.toFixed(2)}%` : 'Unavailable',
      isUp: isFiniteMarketNumber(rtMacroQuotes['BTC']?.change) ? rtMacroQuotes['BTC']!.change >= 0 : true,
    },
  ];
