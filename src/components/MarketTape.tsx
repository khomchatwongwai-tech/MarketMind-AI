...
export const MarketTape: React.FC<MarketTapeProps> = ({
  ...
  return (
    <div className="bg-[#050505] border-b border-[#242424] px-3.5 py-1.5 flex items-center justify-between select-none text-xs w-full max-w-full">
      {/* Mobile Top Bar (< sm) */}
      <div className="flex sm:hidden items-center justify-between w-full gap-2">
        ...
        <div className="flex items-center gap-1.5 overflow-hidden">
          {QUICK_TICKERS.map((sym) => {
            ...
            return (
              <button
                key={sym}
                onClick={() => onSelectTicker(sym as TickerSymbol)}
                className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold transition flex items-center gap-1 shrink-0 ${
                  isSel
                    ? 'bg-[#D4AF37]/20 text-[#F2D675] border border-[#D4AF37]'
                    : 'bg-[#101010] text-[#9CA3AF] border border-[#242424]'
                }`}
              >
                <span>{sym}</span>
        ...
        {onOpenMenu && (
          <button
            onClick={onOpenMenu}
            className="p-1 rounded bg-[#101010] hover:bg-[#151515] border border-[#242424] text-[#E5E5E5] shrink-0"
            title="Open Menu"
      ...
      <div className="hidden sm:flex items-center gap-2 pr-3.5 border-r border-[#242424] shrink-0">
        <span className="relative flex h-2 w-2">
          {isLive && (
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#22C55E] opacity-75"></span>
          )}
      ...
      {/* Horizontal scrolling ticker ribbon (sm and up) */}
      <div className="hidden sm:flex items-center gap-2 overflow-x-auto no-scrollbar py-0.5 px-2 flex-1 scroll-smooth">
        {tapeQuotes.map((item) => {
