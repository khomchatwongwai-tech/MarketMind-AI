...
export class AlpacaProviderError extends Error {
  constructor(public readonly code: AlpacaFailureCode, message: string) {
    super(message);
...
export class AlpacaMarketDataService {
  ...
  private async request(path: string): Promise<any> {
    ...
    // Enforce rate limiter before making network calls
  ...
  static parseSnapshot(
    ...
    if (
      ![price, bid, ask].every((value) => Number.isFinite(value) && value > 0) ||
      bid > ask * 1.05
    ) {
    ...
    return {
      ...
      timestamp: Date.parse(trade?.t || quote?.t || new Date().toISOString()),
      provider: 'Alpaca IEX',
      feed: 'iex',
      isConsolidated: false,
  ...
  async getSnapshot(symbol: string) {
    ...
    const parsed = AlpacaMarketDataService.parseSnapshot(clean, res);
  ...
  async getLatestTrade(
    symbol: string
  ): Promise<{ symbol: string; price: number; size: number; timestamp: number; provider: 'Alpaca IEX' }> {
    const clean = symbol.toUpperCase().trim();
  ...
  async getLatestQuote(symbol: string): Promise<AlpacaStockQuote> {
    ...
    const parsed = AlpacaMarketDataService.parseSnapshot(clean, {
  ...
  async getBars(symbol: string, timeframe = '5Min', limit = 500): Promise<AlpacaBar[]> {
    ...
    const start = new Date(
      Date.now() - (timeframe.includes('Day') || timeframe.includes('Week') ? 730 : 30) * 86400000
    ).toISOString();
