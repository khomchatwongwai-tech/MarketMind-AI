...
export class ProviderStateRegistry {
  ...
  private state(provider: AIProviderName) {
    const state = this.states.get(provider) || { failures: 0 };
    this.states.set(provider, state);
    return state;
  }

  isCircuitOpen(provider: AIProviderName, now = Date.now()) {
    const state = this.state(provider);
    if (!state.circuitOpenedAt) return false;
    if (now - state.circuitOpenedAt >= this.resetMs) {
  ...
  success(provider: AIProviderName, latencyMs: number) {
    const state = this.state(provider);
    state.failures = 0;
    state.circuitOpenedAt = undefined;
  ...
  failure(provider: AIProviderName) {
    const state = this.state(provider);
    state.failures += 1;
    if (state.failures >= this.failureThreshold) state.circuitOpenedAt = Date.now();
  }

  health(provider: AIProviderName, configured: boolean): ProviderHealth {
    const state = this.state(provider);
    const open = this.isCircuitOpen(provider);
    const healthy = configured && !open && state.failures === 0;
    ...
    return {
      ...
      enabled: configured,
      healthy,
      status: open ? 'CIRCUIT_OPEN' : configured ? (state.failures > 0 ? 'DEGRADED' : 'HEALTHY') : 'OFFLINE',
