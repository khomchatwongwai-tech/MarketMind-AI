...
export function aiConfig(env = process.env) {
  ...
  return { enabledProviders: enabled, maxAttempts: Math.min(4, positive(env.AI_MAX_PROVIDER_ATTEMPTS, 2)), timeoutMs: positive(env.AI_REQUEST_TIMEOUT_MS, 15_000), maxOutputTokens: Math.min(8_192, positive(env.AI_MAX_OUTPUT_TOKENS, 1_200)), dailyRequestLimit: positive(env.AI_DAILY_REQUEST_LIMIT, 1_000), cacheEnabled: env.AI_CACHE_ENABLED !== 'false', apiKey: (provider: AIProviderName) => env[keyByProvider[provider]], model: (provider: AIProviderName) => env[`${provider.toUpperCase()}_MODEL`] || modelByProvider[provider] };
}
